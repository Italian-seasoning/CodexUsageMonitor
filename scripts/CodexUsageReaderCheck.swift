...
struct CodexUsageReaderCheck {
    ...
    private static func testSubagentFilesGroupBySessionID() throws {
        try withFixture(named: "subagent-session-id") { root in
            ...
            try expectEqual(snapshot.lifetime.total, 170, "subagent grouped lifetime")
            try expectEqual(snapshot.sessionCount, 1, "subagent grouped session count")
            try expectEqual(snapshot.turnCount, 3, "subagent grouped turns")
            try expectEqual(snapshot.todaySessionCount, 1, "subagent daily session count")
            try expectEqual(snapshot.currentSession.total, 170, "subagent current grouped session")
        }
    }
